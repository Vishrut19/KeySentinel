...
function cmdScanPush() {
    ...
    // Git pre-push hook: scan commits being pushed
    // We compare the current branch with its remote counterpart
    ...
    try {
        ...
        // Try to find the remote branch to compare against
        try {
            // Fetch latest remote refs (non-blocking)
            try {
                runGit(gitRoot, ["fetch", remoteName, "--quiet"]);
            }
            catch {
                // Ignore fetch errors
            }
            ...
            // Remote branch doesn't exist, try main/master
            try {
                ...
                try {
                    ...
                    // No remote branch found - scan last 10 commits as fallback
    ...
    console.error("");
    console.error("Remove or allowlist these before pushing. See .keysentinel.yml allowlist.");
    console.error("");
    if ((0, scanner_1.shouldFail)(allFindings, config.failOn)) {
        process.exit(1);
    }
